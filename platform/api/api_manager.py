...
def build_autogen(args):
    ...
    dynamic_path = args.dynamic_directory
    problems = api.problem.get_all_problems(show_disabled=True)
    for problem in problems:
        if problem.get("autogen", False):
            api.autogen.build_problem_instances(problem["pid"], instances, dynamic_path[0])
...
def main():
    ...
    parser_autogen_build.add_argument("dynamic_directory", nargs=1, help="Directory where dynamic php problem content is stored")
