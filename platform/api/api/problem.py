...
problem_schema = Schema({
    ...
    "php": check(
        ("A problem should either be for php files or not, True/False", [
            lambda php: type(php) == bool])),
...
def get_all_problems(category=None, show_disabled=False):
    ...
    all_prob_list = list(db.problems.find(match, {"_id":0}).sort('score', pymongo.ASCENDING))
    # for problem in all_prob_list:
    #     pid = problem["pid"]
    #     solves = len(get_submissions(pid=pid,correctness=True))
...
def get_unlocked_problems(tid, category=None):
    ...
    for problem in unlocked:
        ...
        solves = len(get_submissions(pid=problem["pid"],correctness=True))
