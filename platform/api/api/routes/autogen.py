...
def serve_autogen_hook(path):
    ...
    php = request.args.get("php", "false") == "true"
    ...
    is_php = api.problem.get_problem(pid=pid)["php"]
    ...
    if is_php:
        return send_from_directory(instance_path, path, mimetype="application/octet-stream", as_attachment=True)
    if mime == 'text/html':
