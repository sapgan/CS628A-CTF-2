...
def shell_accounts_available():
    ...
    return db.ssh_free.find({"tid": {"$exists": False}}).count() > 0

def get_free_new_shell():
    """
        Returns a new unassigned shell from a pool of shells.

        Returns:
            Details of a new shell
    """

    db = api.common.get_conn()
    if(db.ssh_free.find({"tid": {"$exists": False}}).count() <= 0):
        raise InternalException("No new shells available")
    new_shell = db.ssh_free.find({"tid": {"$exists": False}})[0]
    db.ssh_free.remove(new_shell)
    return new_shell
