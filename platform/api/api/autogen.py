...
def build_problem_instances(pid, instances, dynamic_path):
    """
    ...
    dynamic_instance_path = get_dynamic_instance_path(pid, dynamic_path)
    ...
    for n in range(instances):
        ...
        # autogen_instance_dynamic_path = get_dynamic_instance_path(pid, dynamic_path, n=n)

        # if(is_php):
        #     autogen_instance_path = autogen_instance_dynamic_path
...
def get_instance_number(pid, tid):
    """
    Maps the token to an instance number for a problem.
...
def get_dynamic_instance_path(pid, dynamic_directory, n="", public=True):
    """
    Gets the path to the dynamic resources of a problem.Used for only php problems.

    Args:
        pid: the problem id
    Returns:
        The path to the dynamic php resources of an autogen problem.
    """
    problem = api.problem.get_problem(pid=pid)
    ...
    generator_path = path.dirname(problem["generator"])
    name = problem["name"]
    return path.abspath(path.join(dynamic_directory, generator_path, "instances", name, str(n)))
