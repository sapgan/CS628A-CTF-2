...
def check_invalid_autogen_submissions():
    ...
    for problem in api.problem.get_all_problems():
        good_flags = set()
        if ('autogen' in problem and problem['autogen']) or (problem['php']):
            ...
            for flag in incorrect_flags:
                if flag['key'] in good_flags:
                    ...
                    orig_sub = db.submissions.find_one({'key':flag['key'],'correct':True})
                    orig_tid = orig_sub['tid']
                    orig_team = api.team.get_team(tid=orig_tid)
                    if team['eligible'] and team['tid'] not in badteams:
                        if solution is None:
                            cheaters.append((team['team_name'], get_score(tid=flag['tid']), problem['name'], flag['key']))
                        else:
                            cheaters.append((team['team_name'], get_score(tid=flag['tid']), problem['name'], flag['key'], flag['timestamp'], solution['key'], solution['timestamp'], orig_team['team_name']))
...
def grade():
    outputfile = "marks.csv"
    final_marks = open("final.csv","w")
    # web1_log = open("logs/web1_access.log","r")
    # web2_log = open("logs/web2_access.log","r")
    # dungeon_log = open("logs/dungeon.logs","r")
    # master_access_log = open("logs/master_access.log","r")
    # master_sql_log1 = open("logs/master_rev1.sql.log","r")
    # master_sql_log2 = open("logs/master_rev2.sql.log","r")
    # master_sql_log3 = open("logs/master_rev3.sql.log","r")
    # master_sql_log4 = open("logs/master_rev4.sql.log","r")
    # writeup_teams = ["Anonymous","HumBeerHainBC","No one said it would be easy.","Illuminati","CURRENTTEAM","TEAM141","pgd#kh","John_Wick","FunWithFlags","sshh","mercury","Pirates","Annoyance","AbhiAtul","Darksoul","bakri","Deadbeef","KnockKnock","invincible","ekBottleVodka","lalcheetah","Majdoor_shang","terminator","Codex","root","NoMan","GoForkYourself","loneranger","Akatsuki","babajiKiBooty","androxous","carpe_diem","snekarmy","Bit_Bangers","spartans","Encoders","0xhex","AaoKabhiHaveliPe","unlock","lannister","Brogrammers","Junglee"]
    writeup_teams = ["single","#include","gtopctf","unlock","Codex","androxous"]
    ...
    for team in teams:
        ...
        if(team_name not in writeup_teams):
            continue
        ofp.write("-----------------------------\n")
        ofp.write("-----------------------------\n")
        ofp.write(team['team_name']+"\n")
        tid = team["tid"]
        t_users = list(db.users.find({'tid':tid}))
        for user in t_users:
            ofp.write(user['roll']+"\n")
        submissions = api.problem.get_submissions(tid=tid,correctness=True)
        total = 0
        for submission in submissions:
            pid = submission['pid']
            problem = api.problem.get_problem(pid=pid)
            prob_name = problem['name']
            prob_score = problem['score']
            ip = submission['ip']
            total = total+int(problem['score'])
            if(prob_name=="Rule of 5"):
                access_count = os.popen("cat logs/web1_access.log|grep '"+ip+"'|wc -l").read()
                ofp.write("{} {} {} {}\n".format(prob_name,prob_score,ip,access_count))
            elif(prob_name=="Awesome Movie Search"):
                access_count = os.popen("cat logs/web2_access.log|grep '"+ip+"'|wc -l").read()
                ofp.write("{} {} {} {}\n".format(prob_name,prob_score,ip,access_count))
            elif(prob_name=="Enter the Dungeon"):
                access_count = os.popen("cat logs/dungeon.logs|grep '"+ip+"'|wc -l").read()
                ofp.write("{} {} {} {}\n".format(prob_name,prob_score,ip,access_count))
            else:
                ofp.write("{} {} {}\n".format(prob_name,prob_score,ip))
        ofp.write("Total: {}\n".format(total))
    # for user in users:
    # #     print(user)
    #     tid = user['tid']
